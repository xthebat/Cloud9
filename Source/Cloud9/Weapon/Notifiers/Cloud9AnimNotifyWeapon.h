...
protected:
	...
	static WeaponClassType* GetSelectedWeapon(USkeletalMeshComponent* MeshComp)
	{
		...
		RETURN_IF_FAIL(IsValid(Character), nullptr, Error, "Character is invalid");

		let Inventory = Character->GetInventoryComponent();
		RETURN_IF_FAIL(IsValid(Inventory), nullptr, Error, "Inventory is invalid");

		let SelectedWeapon = Inventory->GetSelectedWeapon();
		RETURN_IF_FAIL(IsValid(SelectedWeapon), nullptr, Error, "Selected weapon is invalid");
		RETURN_IF_FAIL(SelectedWeapon->IsWeaponDefined(), nullptr, Error, "Selected weapon is not defined");
